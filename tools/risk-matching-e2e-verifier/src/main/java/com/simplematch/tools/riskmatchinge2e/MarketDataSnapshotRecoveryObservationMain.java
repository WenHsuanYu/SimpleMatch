...
/** Command-line adapter that records a public market-data reconnect and resubscribe. */
public final class MarketDataSnapshotRecoveryObservationMain {
  private MarketDataSnapshotRecoveryObservationMain() {}

  /** Observes two public snapshots separated by a terminated subscription. */
    ...
    final var recovery =
        MarketDataSnapshotRecoveryObserver.observe(
            ...
            new MarketDataSnapshotRecoveryObserver.RecoverySignals(
                optionalPath(values, "--initial-subscription-ready"),
                optionalPath(values, "--initial-snapshot-ready"),
                optionalPath(values, "--disconnected-ready"),
                optionalPath(values, "--replacement-ready"),
                optionalPath(values, "--reconnected-ready"),
                optionalPath(values, "--resubscribed-snapshot-ready")));
    final Path parent = evidence.getParent();
    if (parent != null) {
      java.nio.file.Files.createDirectories(parent);
    }
    new ObjectMapper().findAndRegisterModules().writerWithDefaultPrettyPrinter()
        .writeValue(evidence.toFile(), recovery);
  }
