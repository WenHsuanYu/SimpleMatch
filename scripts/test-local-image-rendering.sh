...
append_lock_entry() {
  ...
  printf '%s|%s:local|%s/%s:local|%s/%s@%s\n' \
    "$service" "$repository" "$registry_endpoint" "$repository" "$registry_endpoint" "$repository" "$digest" >>"$file"
}
...
# Step 5: registry rendering must replace every selected application image with
# the exact digest reference from the lock.
bash "$renderer" \
  --transport registry \
...
grep -Fq "image: ${registry_endpoint}/${matching_repository}@${digest}" "$matching_manifest"
...
# Step 3/5 compatibility boundary: kind-load rendering must preserve the tracked
# local image references and must not require an image lock.
bash "$renderer" \
  --transport kind-load \
  --namespace "$namespace" \
...
# Default rendering remains registry-backed.
bash "$renderer" \
  --image-lock "$matching_lock" --namespace "$namespace" \
  --output "$temp_dir/default.yaml" >/dev/null
grep -Fq "image: ${registry_endpoint}/${matching_repository}@${digest}" "$temp_dir/default.yaml"

if bash "$renderer" --transport invalid --namespace "$namespace" \
    --output "$temp_dir/invalid.yaml" >/dev/null 2>&1; then
  printf '%s\n' 'renderer unexpectedly accepted an invalid image transport' >&2
  exit 1
fi

# A renderer failure must not destroy a previously valid output file.
...
printf '%s\n' 'Local transport-aware manifest rendering contract passed.'
