...
The scenario replaces the Account, Persistence, and QuickFIX Gateway Pods, then
replaces PostgreSQL and one Kafka broker. It proves that durable consumer
positions survive each restart, unresolved quarantines remain absent, QuickFIX
has no stranded delivery intent, and matching.events end offsets do not change
while no new traffic is admitted.

This is restart certification. It does not claim that records were produced while
PostgreSQL or Kafka was unavailable, and it does not replace FIX disconnect and
...
mkdir -p "$evidence_dir/before" "$evidence_dir/after-consumers" \
  "$evidence_dir/after-postgres" "$evidence_dir/after-kafka"
...
wait_workloads() {
  ...
  kns rollout status deployment/quickfix-gateway --timeout="${timeout_seconds}s" >/dev/null
...
capture_events_offsets() {
  ...
  kns exec "$broker" -- /opt/kafka/bin/kafka-get-offsets.sh \
    --bootstrap-server kafka:9092 --topic matching.events \
    | jq -Rn '
...
capture_pod_uids() {
  ...
  (( $(jq 'length' "$destination") > 0 )) || die "selector has no Pods: $selector"
}
...
wait_all_pods_replaced() {
  ...
  for _ in $(seq 1 "$timeout_seconds"); do
    ...
    if jq -e -n --argjson expected "$expected" \
...
wait_named_pod_replaced() {
  ...
  for _ in $(seq 1 "$timeout_seconds"); do
    ...
    ready="$(kns get pod "$name" -o json 2>/dev/null \
...
require_healthy_state() {
  ...
  ' "$state" >/dev/null ||
    die 'baseline requires processed traffic, zero unresolved quarantines, and zero pending FIX intents'
}
...
kns rollout restart deployment/account-service deployment/persistence \
  deployment/quickfix-gateway >/dev/null
wait_workloads
...
require_same_json "$evidence_dir/before/database-state.json" \
...
kns delete pod "$postgres" --wait=false >"$evidence_dir/after-postgres/delete.txt"
...
require_same_json "$evidence_dir/before/matching-events-offsets.json" \
  "$evidence_dir/after-postgres/matching-events-offsets.json" \
  'matching.events advanced during the bounded no-traffic PostgreSQL restart'
...
require_same_json "$evidence_dir/before/matching-events-offsets.json" \
  ...
  'matching.events offsets changed during the bounded no-traffic Kafka restart'
...
jq -n \
  ...
  '{
    ...
    claim:"durable restart behavior with no admitted traffic during dependency replacement",
    proven:[
      "Account consumer Pods replaced and reconstructed durable progress",
      "Persistence consumer Pods replaced and reconstructed durable progress",
      "QuickFIX Gateway Pods replaced and reconstructed durable progress",
      "PostgreSQL Pod replacement preserved consumer state",
      "one Kafka broker Pod replacement preserved consumer state",
      "matching.events end offsets remained stable during bounded no-traffic restarts",
      "no unresolved consumer quarantine remained",
      "no pending QuickFIX delivery intent remained"
    ],
    notProven:[
      "record production while PostgreSQL is unavailable",
      "record consumption while Kafka is unavailable",
      "FIX disconnect and resend across a retained client session"
    ]
