...
class V2ContractValidatorTest {
  ...
  @DisplayName("invalid identifiers, timestamps, currency, quantities, and prices fail at the v2 command seam")
  @Test
  void rejectsInvalidBusinessValues() {
    assertThrows(DomainValidationException.class, () -> validator.validate(newOrder().toBuilder()
        .setCommandId("not-a-uuid")
        .build()));
    assertThrows(DomainValidationException.class, () -> validator.validate(newOrder().toBuilder()
