...
public final class V1OrderCommandAdapter {
  ...
  public NewOrderCommand toNewOrder(OrderCommand command) {
    ...
    final NewOrderCommand.Builder builder = NewOrderCommand.newBuilder()
        ...
        .setQuantity(com.simplematch.contracts.orders.v2.ShareQuantity.newBuilder()
        ...
        .setSessionState(com.simplematch.contracts.common.v2.SessionState.SESSION_STATE_CONTINUOUS)
    ...
    if (command.getOrderType() == com.simplematch.contracts.common.v1.OrderType.ORDER_TYPE_LIMIT) {
      ...
      builder.setLimitPrice(com.simplematch.contracts.common.v2.TwdPrice.newBuilder().setUnits(price.units()));
      builder.setEstimatedNotional(com.simplematch.contracts.common.v2.TwdNotional.newBuilder()
  ...
  public CancelOrderCommand toCancelOrder(OrderCommand command) {
    ...
    final CancelOrderCommand converted = CancelOrderCommand.newBuilder()
        .setMetadata(toV2Metadata(command))
        .setCommandId(command.getCommandId())
        .setOrderId(command.getOrderId())
        .setAccountId(command.getAccountId())
  ...
  public OrderCommand toV1(NewOrderCommand command) {
    ...
    final OrderCommand.Builder builder = baseV1Command(command.getMetadata(), command.getCommandId(), command.getOrderId(),
        command.getAccountId(), command.getSenderCompId(), command.getTargetCompId(), command.getClOrdId(),
        command.getInstrument().getSymbol())
  ...
  public OrderCommand toV1(CancelOrderCommand command) {
    validator.validate(command);
    return baseV1Command(command.getMetadata(), command.getCommandId(), command.getOrderId(), command.getAccountId(),
        command.getSenderCompId(), command.getTargetCompId(), command.getClOrdId(), command.getInstrument().getSymbol())
        .setOrigClOrdId(command.getOrigClOrdId())
        .setSide(toV1Side(command.getSide()))
        .setQuantity("0")
  ...
  private OrderCommand.Builder baseV1Command(
  ...
  private com.simplematch.contracts.common.v2.Side toV2Side(com.simplematch.contracts.common.v1.Side side) {
    return switch (side) {
  ...
  private com.simplematch.contracts.common.v1.Side toV1Side(com.simplematch.contracts.common.v2.Side side) {
    return switch (side) {
