...
| DT-020 | The live kind registry/resource smoke builds the canonical cluster successfully, then the manifest-render phase exits because `--transport` is an unknown option | The registry-only API removed the transport selector, but one live integration caller still used the superseded CLI after deterministic transport contracts had already changed | When removing a deployment CLI option, search every live smoke/certification caller for the removed flag and keep a deterministic contract that rejects the old interface before starting kind | Remove the stale caller argument and use the canonical selector-free renderer; do not restore a compatibility flag merely to make the smoke green, then rerun the complete kind smoke | 2026-08-22 |
