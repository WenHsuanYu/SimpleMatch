...
| [#156](https://github.com/WenHsuanYu/SimpleMatch/issues/156) Debezium task reassignment across Connect workers | `PARTIAL` | The shared baseline-aware CDC verifier, Connect-after-prerequisites phase, spread/toleration manifest contract, and fail-closed worker-loss runner/static fixtures are implemented. Run `scripts/run-local-connect-worker-loss.sh` once against a retained source-aligned disposable namespace to capture task-id-preserving worker/Pod-UID reassignment and exact post-transition Account CDC evidence; until that focused runtime report exists, #156 remains open. The report will be consumed by parent #151 and cannot be relabelled as a parent PASS. |
