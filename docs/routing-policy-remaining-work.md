...
with an explicitly matching run-id, confirms a stable control plane before fault injection, captures
exact runtime identity, and writes diagnostic-only evidence without rerunning the complete
production-like runner. PostgreSQL evidence requires the original slot-0 Pod, RWO PVC/PV, and a
durable row in Flyway-owned `risk_service.local_resilience_marker` after worker return; the row is
removed after observation so repeated diagnostics do not accumulate state. Worker-stop also records
that no PostgreSQL replacement Pod was assigned to another worker before return, and it must not write
observer-owned `risk_service.cdc_delivery_lag`. Kafka evidence requires all three fixed ordinal identities,
...
| [#156](https://github.com/WenHsuanYu/SimpleMatch/issues/156) Debezium task reassignment across Connect workers | `PARTIAL` | The shared baseline-aware CDC verifier, Connect-after-prerequisites phase, spread/toleration manifest contract, and fail-closed worker-loss runner/static fixtures are implemented. The runner now has one bounded runtime precondition adapter: it resolves the image and node selector from the deployed Connect workload, checks the same node-local identity and a containerd execution probe on every eligible kind node before Pod mutation, and retains only the resulting claim evidence. Setup has a separate bounded 300-second budget; the single `--deadline-seconds` value (default 600, maximum 900) starts only at fault injection, so image checks cannot consume the recovery window. The bounded attempts under `out/resilience/connect-worker-loss-20260906-r1/` through `-r8/` are retained; r8 proved Pod deletion, task-id-preserving worker/Pod-UID reassignment, durable Account transition, and exact Kafka record checks, but its pre-fix report checker rejected a legal publication at the Kafka end-offset boundary. The boundary fix and regression contract are in `e666871`, but the prior evidence is not relabelled as PASS and no additional high-cost runtime run is started automatically. One fresh user-authorized focused run remains required to produce a source-aligned PASS report. The report will be consumed by parent #151 and cannot be relabelled as a parent PASS. The image-cache contract fixture uses a deterministic synthetic digest only; production-like runs record the identity observed from node containerd. An active REST connector-config cross-check and the fresh runtime report remain follow-up gates. |
...
The worker-loss diagnostic now composes two narrow preconditions instead of mirroring the whole
deployment specification. The shared focused preflight validates retained source/runtime ownership;
the image-cache adapter validates only the deployed Connect image identity and executable cache on
eligible nodes. Its evidence is linked from the schema-versioned report, while the recovery clock is
started immediately before the UID-guarded Pod deletion. The fixture test uses a synthetic digest so
it remains deterministic; the real runner records node-observed identities. This keeps tool-level
guarantees in their adapters and leaves application behavior (reassignment and CDC publication) to
the focused runtime gate. The image-cache check is therefore a deployment precondition supporting
