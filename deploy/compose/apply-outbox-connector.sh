...
CONNECTOR_PATH="${1:?usage: apply-outbox-connector.sh CONNECTOR_JSON}"
...
if [[ "${CONNECTOR_PATH}" != /* ]]; then
  CONNECTOR_PATH="${SCRIPT_DIR}/${CONNECTOR_PATH}"
fi
...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to apply an outbox connector" >&2
  exit 1
fi
...
CONNECTOR_NAME="$(jq -r '.name // empty' "${CONNECTOR_PATH}")"
if [[ -z "${CONNECTOR_NAME}" ]]; then
  echo "Connector config must contain a non-empty name" >&2
  exit 1
fi
