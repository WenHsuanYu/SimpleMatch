...
public record MarketInstrument(
  ...
  public MarketInstrument {
    ...
    if (referencePriceUnits <= 0 || lowerPriceLimitUnits <= 0 || upperPriceLimitUnits <= 0) {
      throw new MarketSnapshotValidationException("reference price and price limits must be positive");
    }
    if (lowerPriceLimitUnits >= referencePriceUnits || upperPriceLimitUnits <= referencePriceUnits) {
    ...
    if (!tickTable.accepts(referencePriceUnits)
      ...
      throw new MarketSnapshotValidationException("reference price and price limits must align to the tick table");
    }
