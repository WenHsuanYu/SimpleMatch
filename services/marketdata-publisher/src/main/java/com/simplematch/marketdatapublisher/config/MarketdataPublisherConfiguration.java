...
public class MarketdataPublisherConfiguration {
  ...
  DataSource dataSource(PostgresProperties properties) {
    final PostgresJdbcUrl parsed = PostgresJdbcUrl.parse(properties.dsn());
  ...
  MarketSnapshotReadinessHealthIndicator marketSnapshotReadyHealthIndicator(
      MarketSnapshotRepository marketSnapshotRepository, Clock marketdataPublisherClock) {
    return new MarketSnapshotReadinessHealthIndicator(
        marketSnapshotRepository, marketdataPublisherClock);
  }
}
