...
public final class MarketSnapshotImportService {
  ...
  public PreparedMarketSnapshot prepare(byte[] sourceBytes) {
    ...
    final List<MarketInstrument> instruments = safeList(source.instruments()).stream()
        .map(this::normalizeInstrument)
        .sorted(Comparator.comparing(MarketInstrument::symbol).thenComparing(MarketInstrument::venueMic))
        .toList();
    requireDistinctInstrumentIdentities(instruments);
    final CanonicalSnapshot canonical = new CanonicalSnapshot(
        source.sourceIdentity(), source.sourceTimestampUnixMs(), tradingDay.toString(), instruments);
  ...
  private SourceDocument read(byte[] sourceBytes) {
    try {
      ...
      throw new MarketSnapshotValidationException("market snapshot source must be valid JSON", exception);
    }
  }
  ...
  private MarketInstrument normalizeInstrument(SourceInstrument source) {
    ...
    final TickTable tickTable = new TickTable(safeList(source.tickBands()).stream()
        .map(band -> new TickBand(
            band.upperExclusive() == null ? null : parsePrice(band.upperExclusive(), "tick band upper boundary"),
            parsePrice(band.tickSize(), "tick size")))
        .toList());
    final EligibilityReason eligibilityReason = eligibility(source.venueMic(), source.securityType());
  ...
  private long parsePrice(String value, String fieldName) {
    try {
      ...
      throw new MarketSnapshotValidationException(fieldName + " must be a positive TWD price", exception);
    }
  }
  ...
  private LocalDate parseDate(String value, String fieldName) {
    ...
    try {
      ...
      throw new MarketSnapshotValidationException(fieldName + " must be an ISO-8601 date", exception);
    }
  }
  ...
  private String write(CanonicalSnapshot canonical) {
    try {
      ...
      throw new MarketSnapshotValidationException("failed to serialize normalized snapshot", exception);
    }
  }
  ...
  private static final class TaiwanTradingCalendar {
    ...
    private TaiwanTradingCalendar(List<String> rawHolidays) {
      this.holidays = rawHolidays.stream()
          .map(value -> parseHoliday(value))
          .toList();
    }
