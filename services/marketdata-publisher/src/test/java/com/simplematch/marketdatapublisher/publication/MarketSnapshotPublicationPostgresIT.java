...
class MarketSnapshotPublicationPostgresIT {
  ...
  void setUp() {
    ...
    Assumptions.assumeTrue(
        !dsn.isBlank(), "set -D" + DSN_PROPERTY + " to run PostgreSQL verification");
    dataSource = new DriverManagerDataSource(dsn);
    jdbcTemplate = new JdbcTemplate(dataSource);
    assertThat(schemaExists())
        .as("marketdata_publisher must not pre-exist for an isolated verification run")
        .isFalse();
    Flyway.configure()
  ...
  void commitsPublicationAgainstPostgreSql() throws Exception {
    ...
    final MarketSnapshotApplicationService service =
        new MarketSnapshotApplicationService(
    ...
    assertThat(
            jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM marketdata_publisher.outbox", Integer.class))
        .isEqualTo(1);
  }
  ...
  private com.simplematch.marketdatapublisher.snapshot.PreparedMarketSnapshot fixture()
      throws Exception {
    try (InputStream input =
        getClass().getClassLoader().getResourceAsStream("fixtures/xtai-and-roco-snapshot.json")) {
