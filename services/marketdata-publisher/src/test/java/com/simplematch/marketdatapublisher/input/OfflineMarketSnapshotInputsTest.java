...
class OfflineMarketSnapshotInputsTest {
    private final MarketSnapshotImportService importer = new MarketSnapshotImportService(new ObjectMapper());

    @DisplayName("fixture replay and simulator inputs return the same prepared immutable snapshot without network access")
    @Test
    void replayAndSimulatorAreDeterministicAndOffline() {
        final FixtureReplayMarketSnapshotInput replay = new FixtureReplayMarketSnapshotInput(
                importer,
                () -> resource("fixtures/xtai-and-roco-snapshot.json"));
