...
class MarketSnapshotReadinessHealthIndicatorTest {
  private static final Clock CLOCK = Clock.fixed(Instant.parse("2026-07-27T01:00:00Z"), ZoneOffset.UTC);
  ...
  @DisplayName("readiness is up only for the active snapshot matching the current Taiwan trading day")
  @Test
  void isUpForCurrentTaiwanTradingDay() {
    final PublishedMarketSnapshot snapshot = snapshot(LocalDate.of(2026, 7, 27));
    final MarketSnapshotReadinessHealthIndicator indicator =
        new MarketSnapshotReadinessHealthIndicator(new StubRepository(Optional.of(snapshot)), CLOCK);
  ...
  void isOutOfServiceForInvalidCurrentSnapshot() {
    final PublishedMarketSnapshot snapshot = new PublishedMarketSnapshot(
        UUID.randomUUID(), LocalDate.of(2026, 7, 27), 1, "source", 1, "a".repeat(64), "{}", true,
        CLOCK.instant());
    final MarketSnapshotReadinessHealthIndicator indicator =
        new MarketSnapshotReadinessHealthIndicator(new StubRepository(Optional.of(snapshot)), CLOCK);
  ...
  private PublishedMarketSnapshot snapshot(LocalDate tradingDay) {
    return new PublishedMarketSnapshot(
        UUID.randomUUID(), tradingDay, 1, "source", 1, PreparedMarketSnapshot.checksumFor("{}"), "{}", true,
        CLOCK.instant());
  }
  ...
  private static final class StubRepository implements MarketSnapshotRepository {
    ...
    @Override
    public Optional<PublishedMarketSnapshot> findBySourceIdentityAndChecksum(String sourceIdentity, String checksum) {
