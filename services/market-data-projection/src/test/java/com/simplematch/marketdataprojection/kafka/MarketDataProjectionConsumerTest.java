...
/** Verifies that market-data delivery may retry or dead-letter without blocking matching authority. */
class MarketDataProjectionConsumerTest {
  ...
  void routesAnInvalidKeyToTheNonCriticalDeadLetterPathAndCommitsItsSourceOffset() {
    ...
    consumer.onMatchingEvent(record("a".repeat(64), 7L), acknowledgment);

    verify(acknowledgment).acknowledge();
    assertThat(deadLetters.evidence)
        .singleElement()
        .satisfies(value -> assertThat(value.record().eventId()).isEqualTo(EVENT_ID));
  }
  ...
  private NonCriticalDeliveryController controller(RecordingDeadLetterStore deadLetters, int attempts) {
