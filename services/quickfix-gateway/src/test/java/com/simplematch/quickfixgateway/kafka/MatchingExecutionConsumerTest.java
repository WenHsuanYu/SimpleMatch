...
class MatchingExecutionConsumerTest {
  // Verify that a tracked execution event is converted into the corresponding FIX Execution Report
  // and sent back to the original session.
  // Scenario: the registry already contains an accepted new order; receiving EXECUTION_TYPE_NEW
  // should generate the standard report.
  ...
  void executionEventBuildsExecutionReportForTrackedOrder() throws Exception {
    ...
    final MatchingExecutionConsumer consumer =
  ...
  // Scenario: receive a CANCEL_REJECTED event and, when the registry can resolve the original order
  // state, send a 35=9 message.
  ...
  // Verify that an execution event missing required enrichment fails immediately to avoid sending
  // an incomplete FIX message.
  // Scenario: build an execution event with only the base fields, then confirm the consumer throws
  // and does not call the sender.
