...
public final class FixMessageMapper {
  ...
  public ExecutionReport buildPendingNew(FixOrderSnapshot order, FixExecutionIdentity execution) {
    ...
    report.setChar(quickfix.field.Side.FIELD, mapSide(order.side()));
    report.setString(LeavesQty.FIELD, normalizeDecimal(order.quantity().value()));
  ...
  public ExecutionReport buildRejected(
    ...
    if (!order.quantity().value().isBlank()) {
      report.setString(OrderQty.FIELD, normalizeDecimal(order.quantity().value()));
    }
  ...
  public Message buildExecutionReport(ExecutionEvent executionEvent, OrderSessionState state) {
    ...
    report.setString(
        LeavesQty.FIELD, fallbackDecimal(executionEvent.getLeavesQty(), state.quantity()));
  ...
  private String transactTime(ExecutionEvent executionEvent) {
    if (executionEvent.hasMetadata() && executionEvent.getMetadata().getCreatedAtUnixMs() > 0) {
      return FIX_TIMESTAMP.format(
          Instant.ofEpochMilli(executionEvent.getMetadata().getCreatedAtUnixMs()));
    }
