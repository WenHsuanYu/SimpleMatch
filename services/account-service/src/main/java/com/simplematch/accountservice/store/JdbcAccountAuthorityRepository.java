...
public class JdbcAccountAuthorityRepository implements AccountAuthorityRepository {
  ...
  private static final RowMapper<AccountLimit> LIMIT_MAPPER =
      (rs, row) ->
          new AccountLimit(
  ...
  private static final RowMapper<AccountPosition> POSITION_MAPPER =
      (rs, row) ->
          new AccountPosition(
              rs.getString("account_id"),
              rs.getString("symbol"),
              ...
              rs.getLong("version"),
              rs.getLong("updated_at_unix_ms"));
  private static final RowMapper<AccountReservation> RESERVATION_MAPPER =
      (rs, row) ->
  ...
  public Optional<AccountReservation> findReservationByRequestId(String requestId) {
    return jdbcTemplate
        .query(reservationSelect() + " WHERE request_id = ?", RESERVATION_MAPPER, requestId)
        .stream()
        .findFirst();
  }
  ...
  public Optional<AccountReservation> findReservationForUpdate(String reservationId) {
    return queryForUpdate(
        reservationSelect() + " WHERE reservation_id = ? FOR UPDATE",
        RESERVATION_MAPPER,
        reservationId);
  }
