...
public class AccountReservationApplicationService implements ReservationService {
        ...
        if (operation.side() == Side.SIDE_BUY) {
            ...
            if (position == null
                    || position.longQuantity()
                            ...
                            .compareTo(operation.quantity()) < 0) {
    ...
    public ReservationRecord release(ReleaseReservationOperation operation) {
        ...
        final AccountReservation reservation = accounts.findReservationForUpdate(
                        identity.reservationId().value())
    ...
    public ReservationRecord applyFill(ApplyFillOperation operation) {
        ...
        if (!accounts.claimInbox(
                ...
                clock.millis())) {
            return accounts.findReservationForUpdate(identity.reservationId().value()).map(this::toLegacy)
                    .orElseThrow(() -> new IllegalArgumentException("reservation not found"));
        }
        final AccountReservation reservation = accounts.findReservationForUpdate(identity.reservationId().value())
                .orElseThrow(() -> new IllegalArgumentException("reservation not found"));
        if (!reservation.requestId().equals(identity.requestId().value())
                || !reservation.orderId().equals(identity.orderId().value())) {
        ...
        final AccountReservation changed = new AccountReservation(
                ...
                reservation.filledQuantity().add(fill.quantity().value()), reservation.limitPrice(),
        ...
        emit(changed, status == ReservationStatus.RESERVATION_STATUS_APPLIED
                ...
                : AccountLifecycleState.ACCOUNT_LIFECYCLE_STATE_RESERVED, "", now);
        return toLegacy(changed);
    }
    ...
    private ReservationRecord persistRejected(
        ...
        final AccountReservation rejected = AccountReservation.rejected(
                operation.orderId(), snapshot, reasonCode, reasonText, now);
    ...
    private void releaseCancelledAuthority(AccountReservation reservation, long now) {
        ...
        if (reservation.side() == Side.SIDE_BUY) {
            ...
            final AccountLimit changed = limit.withBalances(
                    limit.reservedNotional().subtract(reservation.reservedNotional()).max(BigDecimal.ZERO),
        ...
        final AccountPosition position = accounts.findPositionForUpdate(
                        reservation.accountId(), reservation.symbol())
                .orElseThrow(() -> new IllegalStateException("position is not provisioned"));
        final BigDecimal released = position.reservedLongQuantity().min(reservation.remainingQuantity());
        final AccountPosition changed = new AccountPosition(
                position.accountId(),
                position.symbol(),
                position.longQuantity(),
    ...
    private void emit(AccountReservation reservation, AccountLifecycleState state, String reasonCode, long now) {
        ...
        final AccountLifecycleEvent event = AccountLifecycleEvent.newBuilder()
                ...
                .setReservedNotional(TwdNotional.newBuilder()
                        .setUnits(toFixedUnits(reservation.reservedNotional()))
                        .build())
                .setReservedQuantity(ShareQuantity.newBuilder()
