...
public class AccountReservationApplicationService implements ReservationService {
  ...
  public ReservationRecord applyFill(
    ...
    if (!accounts.claimInbox(
        CONSUMER_NAME, executionId, orderId, aggregateSequence, clock.millis())) {
      return accounts
          .findReservationForUpdate(reservationId)
          .map(this::toLegacy)
          .orElseThrow(() -> new IllegalArgumentException("reservation not found"));
    ...
    if (fillQuantity == null
        || fillQuantity.signum() <= 0
        || fillQuantity.compareTo(reservation.remainingQuantity()) > 0
        || fillPrice == null
        || fillPrice.signum() <= 0) {
    ...
    final AccountReservation changed =
        new AccountReservation(
            ...
            reservation.quantity(),
            remaining,
            reservation.filledQuantity().add(fillQuantity),
            reservation.limitPrice(),
    ...
    if (reservation.side() == Side.SIDE_BUY) {
      ...
      final AccountLimit changed =
          limit.withBalances(
              ...
              fill
                  ? limit.availableNotional().add(releasedNotional).subtract(utilizedNotional)
      ...
      if (fill) {
        ...
        final AccountPosition changedPosition =
            new AccountPosition(
                position.accountId(),
                position.symbol(),
                position.longQuantity().add(quantity),
                position.shortQuantity(),
      ...
      final BigDecimal longQuantity =
          fill ? position.longQuantity().subtract(released) : position.longQuantity();
      final AccountPosition changed =
          new AccountPosition(
              position.accountId(),
              position.symbol(),
