...
public record AccountReservation(
    ...
    public AccountReservation {
        ...
        if (quantity == null || quantity.signum() <= 0 || remainingQuantity == null || remainingQuantity.signum() < 0
                || filledQuantity == null || filledQuantity.signum() < 0
    ...
    public static AccountReservation accepted(
            String reservationId, ReserveOperationSnapshot operation, BigDecimal reservedNotional, long now) {
        return new AccountReservation(reservationId, operation.requestId(), operation.orderId(), operation.accountId(),
                operation.symbol(), operation.side(), operation.quantity(), operation.quantity(), BigDecimal.ZERO,
                operation.limitPrice(), reservedNotional, ReservationStatus.RESERVATION_STATUS_ACCEPTED, "", "", 0, now, now);
    }
    ...
    public static AccountReservation rejected(
            String reservationId, ReserveOperationSnapshot operation, String reasonCode, String reasonText, long now) {
        return new AccountReservation(reservationId, operation.requestId(), operation.orderId(), operation.accountId(),
                operation.symbol(), operation.side(), operation.quantity(), operation.quantity(), BigDecimal.ZERO,
                operation.limitPrice(), BigDecimal.ZERO, ReservationStatus.RESERVATION_STATUS_REJECTED,
                reasonCode, reasonText, 0, now, now);
    }
    ...
    public record ReserveOperationSnapshot(
            String requestId, String orderId, String accountId, String symbol, Side side,
