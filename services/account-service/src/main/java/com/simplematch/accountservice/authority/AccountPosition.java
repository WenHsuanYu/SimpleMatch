...
public record AccountPosition(
  ...
  public AccountPosition {
    ...
    if (reservedLongQuantity.compareTo(longQuantity) > 0 || reservedShortQuantity.compareTo(shortQuantity) > 0) {
  ...
  public static AccountPosition provisioned(String accountId, String symbol, long now) {
    return new AccountPosition(accountId, symbol, BigDecimal.ZERO, BigDecimal.ZERO,
        BigDecimal.ZERO, BigDecimal.ZERO, 0, now);
  }
