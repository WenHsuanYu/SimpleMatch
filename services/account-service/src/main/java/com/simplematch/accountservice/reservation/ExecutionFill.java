 ...
 *
 * <p>The execution identifier is the idempotency identity. The optional aggregate sequence is used
 * to reject stale lifecycle delivery when the producing stream supplies one. Quantity and price use
 * different Java types so they cannot be exchanged at a call site.
 *
  ...
  public record AggregateSequence(Long value) {
    ...
    public AggregateSequence {
      if (value != null && value < 0) {
        throw new IllegalArgumentException("aggregate_sequence must be non-negative when provided");
      }
    }
