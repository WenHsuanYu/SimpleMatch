...
public final class OrderAdmissionValidator {
  ...
  @SuppressWarnings(
      "PMD.CyclomaticComplexity") // Admission validation is deliberately one visible policy
                                  // boundary.
  public AdmissionCommand validate(NewOrderCommand command) {
    ...
    if (!"XTAI".equals(venue) && !"ROCO".equals(venue)) {
      throw new AdmissionValidationException(
          "INVALID_INSTRUMENT", "venue_mic must be XTAI or ROCO");
    }
    if (command.getSide() == Side.SIDE_UNSPECIFIED || command.getQuantity().getShares() <= 0) {
      throw new AdmissionValidationException(
          "INVALID_COMMAND", "side and positive quantity are required");
    }
    if (command.getOrderType() == OrderType.ORDER_TYPE_UNSPECIFIED
      ...
      throw new AdmissionValidationException(
          "INVALID_COMMAND", "order type, time-in-force, and TWD currency are required");
    }
    if (command.getSessionState() != SessionState.SESSION_STATE_CONTINUOUS) {
      throw new AdmissionValidationException(
          "UNSUPPORTED_SESSION", "new-order admission requires CONTINUOUS session");
    }
    final Long limitPrice =
        command.getOrderType() == OrderType.ORDER_TYPE_LIMIT
            ? positive(command.getLimitPrice().getUnits(), "limit_price")
            : null;
    ...
    if (command.getSenderCompId().isBlank()
      ...
      throw new AdmissionValidationException(
          "INVALID_COMMAND", "sender, target, and cl_ord_id are required");
    }
    final UUID routingSnapshotId =
        command.getRoutingSnapshotId().isBlank()
            ? null
            : uuid(command.getRoutingSnapshotId(), "routing_snapshot_id");
    return new AdmissionCommand(
        ...
        command.getQuantity().getShares(),
        limitPrice,
        command.getOrderType().name(),
        command.getTif().name(),
        tradingDay,
  ...
  @SuppressWarnings(
      "PMD.CyclomaticComplexity") // Cancel validation is an indivisible admission policy.
  public AdmissionCommand validateCancel(CancelOrderCommand command) {
    ...
    if (command.getInstrument().getSymbol().isBlank()
      ...
      throw new AdmissionValidationException(
          "INVALID_INSTRUMENT", "symbol and supported venue are required");
    }
    if (command.getSide() == Side.SIDE_UNSPECIFIED
        ...
        || command.getClOrdId().isBlank()) {
      throw new AdmissionValidationException(
          "INVALID_COMMAND", "cancel identity and side are required");
    }
    if (command.getSessionState() != SessionState.SESSION_STATE_CONTINUOUS) {
      throw new AdmissionValidationException(
          "UNSUPPORTED_SESSION", "cancel admission requires CONTINUOUS session");
    }
    final LocalDate tradingDay;
    try {
      tradingDay = LocalDate.parse(command.getTradingDay().getIsoDate());
    ...
    return new AdmissionCommand(
        ...
        accountId,
        command.getInstrument().getSymbol(),
        command.getInstrument().getVenueMic(),
        command.getSide().name(),
        1,
        null,
        "CANCEL",
