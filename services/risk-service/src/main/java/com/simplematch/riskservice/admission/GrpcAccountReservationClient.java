...
public final class GrpcAccountReservationClient implements AccountReservationClient, AutoCloseable {
  ...
  public GrpcAccountReservationClient(PlatformProperties properties) {
    this.channel = ManagedChannelBuilder.forTarget(properties.grpc().targets().accountService())
        .usePlaintext().build();
  ...
  public ReservationOutcome reserve(AdmissionCommand command) {
    final ReserveRequest request = ReserveRequest.newBuilder()
        .setRequestId(command.commandId().toString()).setOrderId(command.orderId().toString())
        .setAccountId(command.accountId().toString()).setSymbol(command.symbol())
        .setSide(command.side().equals(Side.SIDE_SELL.name()) ? Side.SIDE_SELL : Side.SIDE_BUY)
        .setQuantity(Long.toString(command.quantity()))
        .setLimitPrice(command.limitPriceUnits() == null ? "" : fixedPrice(command.limitPriceUnits())).build();
  ...
  private String fixedPrice(long units) {
    return BigDecimal.valueOf(units).movePointLeft(4).setScale(4, RoundingMode.UNNECESSARY).toPlainString();
  }
}
