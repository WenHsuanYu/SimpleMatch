...
class SubmissionValidatorTest {
  ...
  private final SubmissionValidator validator =
      new SubmissionValidator(Clock.fixed(Instant.ofEpochMilli(NOW), ZoneOffset.UTC));
  ...
  void rejectsMissingPriceForLimitOrder() {
    final ResolvedSubmissionCommand command =
        resolvedNewOrder("cmd-1", "O-C1", "C1", "", OrderType.ORDER_TYPE_LIMIT);
  ...
  void acceptsMarketOrderWithoutPrice() {
    final ResolvedSubmissionCommand command =
        resolvedNewOrder("cmd-1", "O-C1", "C1", "", OrderType.ORDER_TYPE_MARKET);
    final SubmissionDecision decision = validator.evaluate(command);
  ...
  void rejectsCancelWithoutOriginalClientOrderId() {
    final SubmissionDecision decision =
        validator.evaluate(resolvedCancelOrder("cmd-1", "O-C1", "CXL-1", ""));
  ...
  void rejectsNonUuidRequestId() {
    final SubmissionDecision decision =
        validator.evaluate(resolvedNewOrder("not-a-uuid", "O-C1", "C1"));
  ...
  void rejectsTypedEmptyNewOrderAfterNormalization() {
    final SubmissionDecision decision =
        validator.evaluate(ResolvedSubmissionCommand.typedEmpty(CommandType.COMMAND_TYPE_NEW));
  ...
  void rejectsMissingSessionIdentity(
      String ignoredCaseName, ResolvedSubmissionCommand command, String expectedReasonCode) {
    final SubmissionDecision decision = validator.evaluate(command);
  ...
  void rejectsOversizedClOrdIdWhileKeepingRawValueForResponse() {
    ...
    final SubmissionDecision decision =
        validator.evaluate(resolvedNewOrder("cmd-1", "O-C1", oversized));
    ...
    assertThat(decision.submission().persistedClOrdId()).hasSize(64).isNotEqualTo(oversized);
  ...
  void rejectsOversizedOrigClOrdIdWhileKeepingRawValueForResponse() {
    ...
    final SubmissionDecision decision =
        validator.evaluate(resolvedCancelOrder("cmd-1", "O-C1", "CXL-1", oversized));
  ...
  private static Stream<Arguments> oversizedIdentifierCases() {
    ...
    return Stream.of(
        ...
        Arguments.of(
            "oversized order_id", resolvedNewOrder("cmd-1", oversized, "C1"), "OVERSIZED_ORDER_ID"),
        Arguments.of(
  ...
  private static ResolvedSubmissionCommand resolvedNewOrderWithSession(
      String commandId, String orderId, String clOrdId, String senderCompId, String targetCompId) {
    return new ResolvedSubmissionCommand(
        SubmissionCommand.create(
            new SubmissionCommand.RequestMetadata(
                normalize(commandId), orderId, "ACC-1", senderCompId, targetCompId, clOrdId, ""),
  ...
  private static ResolvedSubmissionCommand resolvedNewOrderWithSymbol(
      String commandId, String orderId, String clOrdId, String symbol) {
    return new ResolvedSubmissionCommand(
        SubmissionCommand.create(
            new SubmissionCommand.RequestMetadata(
                normalize(commandId), orderId, "ACC-1", "CLIENT", "SIMPLEMATCH", clOrdId, ""),
            new SubmissionCommand.OrderDetails(
                symbol,
                Side.SIDE_BUY,
                "10",
