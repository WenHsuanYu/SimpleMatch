...
class JdbcCdcDeliveryProgressStoreTest {
  ...
  void setUp() {
    ...
    store = new JdbcCdcDeliveryProgressStore(jdbc, transactions);
  }
  ...
  void derivesLagFromRiskOutboxRowsMissingAnExactKafkaObservation() {
    ...
    store.observe(new CdcDeliveryObservation(DELIVERED, "matching.commands", 2, 14, 3_000));
  ...
  void returnsToZeroAfterThePendingOutboxEventIsObserved() {
    ...
    store.observe(new CdcDeliveryObservation(PENDING, "matching.commands", 4, 18, 5_500));
