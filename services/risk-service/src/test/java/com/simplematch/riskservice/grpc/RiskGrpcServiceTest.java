...
class RiskGrpcServiceTest {
  ...
  // Scenario: submit a valid new order and confirm the observer receives completion and the
  // expected order data.
  ...
  // Verify that cancelOrder returns a rejection instead of a gRPC error when originalClientOrderId
  // is missing.
  // Scenario: submit an incomplete cancel order and confirm the response reason code matches the
  // validation rule.
  ...
  // Scenario: send a new order whose command type was mistakenly set to CANCEL and confirm the
  // database stores NEW.
  ...
  void submitOrderNormalizesUnexpectedCommandTypeToNew() {
    ...
    service.submitOrder(
        SubmitOrderRequest.newBuilder()
            .setCommand(
                newNewOrder("cmd-3", "O-C2", "C2").toBuilder()
  ...
  // Scenario: send a cancel order whose command type was mistakenly set to NEW and confirm the
  // database stores CANCEL.
  ...
  void cancelOrderNormalizesUnexpectedCommandTypeToCancel() {
    ...
    service.cancelOrder(
        CancelOrderRequest.newBuilder()
            .setCommand(
                newCancelOrder("cmd-4", "O-C1", "CXL-1", "C1").toBuilder()
    ...
    assertThat(
            jdbcTemplate.queryForObject(
                ...
                String.class,
                "CXL-1"))
        .isEqualTo("COMMAND_TYPE_CANCEL");
  }

  // Verify that submitOrder returns the expected validation rejection code when it receives the
  // default empty command.
  // Scenario: pass OrderCommand.getDefaultInstance() directly and confirm the result is rejected
  ...
  // Verify that cancelOrder returns the expected validation rejection code when it receives the
  // default empty command.
  // Scenario: pass OrderCommand.getDefaultInstance() directly and confirm the cancel flow also
  // returns a consistent rejected result.
  ...
  void submitOrderEchoesRawOversizedClOrdId() {
    ...
    service.submitOrder(
        SubmitOrderRequest.newBuilder()
            .setCommand(
                newNewOrder("cmd-5", "O-C5", "C5").toBuilder().setClOrdId(oversized).build())
            .build(),
        observer);
  ...
  private OrderCommand newCancelOrder(
